...
export const BillingService = {
  ...
  setStorageCapacity: async (subscriptionPlanId:string,currentTeamId?:string,)=>{
    ...
    if (!currentTeamId) {
      const storagePlans = USER_PRICING.flatMap(user => user.plans.map(plan => ({ planId: plan.planId, storage: plan.storage })));
      ...
      const storagePlans = TEAM_PRICING.flatMap(team => team.plans.map(plan => ({ planId: plan.planId, storage: plan.storage })));
